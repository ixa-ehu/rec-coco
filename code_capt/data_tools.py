...
def get_data(model_type, TRAIN, words, EMB, enforce_gen, n_side_pixl):
    ...
    cap_pos, OBJ_ctr_sd, rel_ids, TRAIN_relevant = get_TRAIN_relevant(TRAIN, words)
    ...
    EMBEDDINGS['cap_list'] = list(set(getCaptionWords(TRAIN_relevant['cap'])))
    ...
    EMBEDDINGS['cap_EMB'] = wordlist2emb_matrix(EMBEDDINGS['cap_list'], EMB_dict)
    ...
    return X, X_extra, y, y_pixl, X_extra_enf_gen, X_enf_gen, y_enf_gen, y_enf_gen_pixl, cap_pos, rel_ids, OBJ_ctr_sd, OBJ_ctr_sd_enf_gen, EMBEDDINGS, TRAIN_relevant
...
def relevant_instances2X_and_y(model_type, TRAIN_relevant, EMBEDDINGS, enforce_gen, n_side_pixl):
    ...
    # get X:
    X, X_enf_gen = {}, {}
    X['subj'], X['pred'], X['obj'], X['cap'] = [], [], [], []
    X_enf_gen['subj'], X_enf_gen['pred'], X_enf_gen['obj'], X_enf_gen['cap'] = [], [], [], []
    ...
    if X_enf_gen['subj'] != []:
        ...
        X_enf_gen['subj'], X_enf_gen['pred'], X_enf_gen['obj'], X_enf_gen['cap'] = None, None, None, None
...
def get_TRAIN_relevant(TRAIN, words):
    ...
    TRAIN_relevant, rel_ids, OBJ_ctr_sd, cap_pos = {}, [], [], []
    ...
    for i in range(len( TRAIN['subj'] )): # Samples loop
        ...
        if we_have_it == True:
            ...
            cap_pos_dict = TRAIN['cap_pos'][i]
            cap_pos_dict = cap_pos_dict.split(" ")
            cap_pos_list = [cap_pos_dict[1], cap_pos_dict[3], cap_pos_dict[5].replace('}"', '')]
            cap_pos.append(cap_pos_list)

    OBJ_ctr_sd = np.array(OBJ_ctr_sd)
    print('We have gotten ' + str(len(TRAIN_relevant['subj'])) + ' RELEVANT instances')
    return cap_pos, OBJ_ctr_sd, rel_ids, TRAIN_relevant
