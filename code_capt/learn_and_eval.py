...
def main():
    ...
    saveFolder = '../results/results_REF-COCO_capt' #wd.get_folder_name(args)
    ...
    X, X_extra, y, y_pixl, X_extra_enf_gen, X_enf_gen, y_enf_gen, y_enf_gen_pixl, cap_pos, rel_ids, OBJ_ctr_sd, OBJ_ctr_sd_enf_gen, \
    ...
    for fold_count, (train_idx, test_idx) in enumerate(kf): # FOLDS loop
        ...
        for method in args.method_compare: # METHODS LOOP
            ...
            # --- LEARN the model --- #
            model_s = model.method_learn(X_train, X_extra_train, y_train, y_pixl[aux_train_idx], EMBEDDINGS)
        
            model_s.load_weights('weights.hdf5')
            ...
            y_pred_train = model_s.predict([X_train['subj'], X_train['pred'], X_train['obj'], X_train['cap'], X_extra_train])#X['cap'], X_extra])
            y_pred_test = model_s.predict([X_test['subj'], X_test['pred'], X_test['obj'], X_test['cap'], X_extra_test])#X['cap'], X_extra])
